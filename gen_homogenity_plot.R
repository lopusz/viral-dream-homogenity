...
conv_prob_to_pred<-function(pred_prob,ans) {
  res<-data.frame(t(apply(pred_prob,1, 
                   function(x) find_opt_pred(x,ans,calc_matthews_score))))
  return(res)
}
...
get_sorted_teams_df<-function(pred,ans) {
  pred_correct<-t(ans %*% t(rep(1,nrow(pred))))
  #score<-apply(pred==pred_correct,1,function(x) sum(x))
  score<-apply(pred==pred_correct,1,function(x) calc_matthews_score(x,ans))
...
plot_class<-function(class,sorted_subject_ids_df,sorted_teams_df) {
  ...
  levels(df$Team)<-sprintf("%s [%4.1f]", sorted_teams_df$Team, sorted_teams_df$Score)
...
main<-function(fname) {
  ...
  sorted_teams_df<-get_sorted_teams_df(pred,ans)
  
  class<-conv_pred_to_class(pred,ans)
  
  z<-plot_class(class,sorted_subject_ids_df,sorted_teams_df)
